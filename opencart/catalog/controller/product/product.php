...
$this->data['tproducts'] = array();
$featuredproducts = array();
$featuredproducts = $this->model_catalog_product->getfeaturedProducts($this->request->get['product_id']);
foreach($featuredproducts as $product_id)
{
...
{
$tproduct_info = $this->model_catalog_product->getProduct($product_id);
if($tproduct_info)
{
if ($tproduct_info['image']) {
					$timage = $this->model_tool_image->resize($tproduct_info['image'], $this->config->get('config_image_related_width'), $this->config->get('config_image_related_height'));
				} else {
					$timage = false;
				}
if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
					$tprice = $this->currency->format($this->tax->calculate($tproduct_info['price'], $tproduct_info['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$tprice = false;
				}
if ((float)$tproduct_info['special']) {
					$tspecial = $this->currency->format($this->tax->calculate($tproduct_info['special'], $tproduct_info['tax_class_id'], $this->config->get('config_tax')));
				} else {
					$tspecial = false;
				}
				
				if ($this->config->get('config_review_status')) {
					$trating = (int)$tproduct_info['rating'];
				} else {
					$trating = false;
				}
							
				$this->data['tproducts'][] = array(
					'tproduct_id' => $tproduct_info['product_id'],
					'tthumb'   	 => $timage,
					'tname'    	 => $tproduct_info['name'],
					'tprice'   	 => $tprice,
					'tspecial' 	 => $tspecial,
					'trating'     => $trating,
					'treviews'    => sprintf($this->language->get('text_reviews'), (int)$tproduct_info['reviews']),
					'thref'    	 => $this->url->link('product/product', 'product_id=' . $tproduct_info['product_id'])
				);
			}
}
}
