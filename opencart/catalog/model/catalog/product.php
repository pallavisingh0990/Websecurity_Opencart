...
class ModelCatalogProduct extends Model {
	...
	public function getBestSellerProducts($limit) {
		...
		if (!$product_data) { 
			...
			$query = $this->db->query("SELECT op.product_id, SUM(op.quantity),COUNT(*) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) LEFT JOIN `" . DB_PREFIX . "product` p ON (op.product_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE o.order_status_id > '0' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' GROUP BY op.product_id ORDER BY total DESC LIMIT " . (int)$limit);
...
public function getfeaturedProducts($product_id) {
$servername = "localhost";
$username = "root";
$password = "root";
$dbname = "opencart";
$query = $this->db->query("select distinct product_id from oc_product_to_category where category_id in (select category_id from oc_product_to_category where product_id='$product_id')");
...
//$sql = "select distinct product_id from oc_product_to_category where category_id in (select category_id from oc_product_to_category where product_id='$product_id') limit 3";

//$sql = "select distinct a.product_id from oc_product_to_category a, oc_order_product b where category_id in (select category_id from oc_product_to_category where product_id='$product_id') and (a.product_id =b.product_id) order by total desc limit 3";

//$sql = "select distinct a.product_id,a.category_id,b.quantity,b.total,b.name from oc_product_to_category a, oc_order_product b where category_id in (select category_id from oc_product_to_category where product_id='$product_id') and (a.product_id =b.product_id) order by total desc limit 3";
$sql = "select distinct (a.product_id),b.name,b.quantity,b.total from oc_product_to_category a, oc_order_product b where category_id in (select category_id from oc_product_to_category where product_id='$product_id') and (a.product_id =b.product_id) order by total desc limit 5;";

//$sql = "SELECT distinct(d.product_id) FROM oc_category a, oc_product_to_category b,oc_order_product d WHERE (a.category_id=b.category_id) AND (b.product_id=d.product_id) AND (d.product_id = '$product_id') ORDER BY quantity DESC LIMIT 3";
